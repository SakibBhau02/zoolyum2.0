...
export async function generateMetadata(): Promise<Metadata> {
  return pageMeta("contact", {
    ...
    description: "Tell us where your brand blends in. One guided conversation, a reply within one working day - Zoolyum, brand strategy and digital innovation agency, Mirpur 11, Dhaka.",
    canonical: "/contact",
    ogTitle: "Start a Conversation | Zoolyum",
    ogDescription: "Tell us where your brand blends in. One guided conversation, a reply within one working day - Zoolyum, Mirpur 11, Dhaka.",
    card: "summary",
  });
}
...
async function getQuote() {
  ...
  return wall[1] ?? TS_TESTIMONIALS[1];
}

function buildJsonLd(contact: { email: string; phone: string }, faqs: { q: string; a: string }[]) {
  return {
      ...
      <section className="relative overflow-hidden pb-24 pt-12 md:pb-32 md:pt-16" aria-label="Start a conversation">
        ...
        <div className="section-shell relative z-10 grid gap-10 lg:grid-cols-12">
          <Reveal className="lg:col-span-7">
            <ContactWizard contactEmail={contact.email} services={services.map((s) => ({ name: s.name, tagline: s.tagline, slug: s.slug }))} />
          </Reveal>
